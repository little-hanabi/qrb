...
public:
	...
	Error(short line, Type type, std::string msg = {}) : _msg(std::move(msg)), _line(line), _type(type) {}
	...
	{
		return _type == o._type && _msg == o._msg && _line == o._line;
	}
...
#define FormatError(...) Error(__LINE__, Error::Format, std::string(__VA_ARGS__))
#define ChecksumError(...) Error(__LINE__, Error::Checksum, std::string(__VA_ARGS__))
#define UnsupportedError(...) Error(__LINE__, Error::Unsupported, std::string(__VA_ARGS__))
